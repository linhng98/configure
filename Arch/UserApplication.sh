...
## wallpaper
mkdir ~/Pictures
cp -R Documents/configure/Arch/wallpaper ~/Pictures/
...
## auto start
cp ~/Documents/configure/Arch/config_file/zprofile ~/.zprofile
...
if [ $device_type -eq 2 ] then
    ...
    sudo cp /home/linh/Documents/configure/Arch/config_file/20-intel.conf /etc/X11/xorg.conf.d/20-intel.conf

    ## touchpad
    sudo pacman -S --noconfirm xf86-input-libinput
    sudo cp /home/linh/Documents/configure/Arch/config_file/30-touchpad.conf /etc/X11/xorg.conf.d/30-touchpad.conf
fi
